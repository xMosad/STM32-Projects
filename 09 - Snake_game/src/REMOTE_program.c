...
volatile u8 global_u8FlagRead           = 1 ;
...
{	
	...
	NVIC_voidSetPriority(REMOTE_NVIC_NUMBER , 1);
...
void REMOTE_HANDLER(void) {
	...
	if (global_u8FlagRead){
		/* Get time elapsed*/
		time = STK_u32GetElapsedTime(TIME_US);
		/* Start timer to count till the next isr */
		STK_voidStart();
		/* Wait for the start bit before recording frame*/
		if ((global_u8Flag == 0) && (time > 5000) && (time < 18000)){
			global_u8Flag = 1 ;
			global_u8Index = 0 ;
		}
		/* Start bit came and will start recording the frame */
			...
			/* If bit == 1 */
			if ((time > 2000 ) && (time < 2800 ) && (global_u8Index < 32 )){
				SET_BIT(global_u32Frame , global_u8Index );
				global_u8Index++;
			}
			/* If bit == 0 */
			else if ( (time > 800 ) &&  (time < 1600 ) && (global_u8Index < 32 ) ){
...
static u8 REMOTE_u8CheckFrame(void)
{
	...
	global_u32Frame = 0 ;
	if ( (global_u32Data & global_u32DataInverted) ){
